...
async def send_alert(application, analysis, news_list=None):
    if analysis["direction"] in ["📈 LONG", "📉 SHORT"]:
        ...
        text = (
            f"📡 Auto Signal: {analysis['symbol']} ({analysis['interval']})\n"
            f"Signal: {analysis['direction']}\n"
            f"Confidence: {analysis['confidence']}%\n"
            f"Price: {analysis['price']}\n"
            f"RSI: {analysis['rsi']:.2f} | MACD: {analysis['macd_hist']:.5f}\n"
            f"EMA 50: {analysis['ema_50']:.2f} | EMA 200: {analysis['ema_200']:.2f}\n"
            f"📊 Volume: {analysis['volume']:.0f} | Avg: {analysis['avg_volume']:.0f}\n"
            f"{'🔥 Volume Spike Detected!' if analysis['volume_spike'] else ''}\n"
            f"Entry: {analysis['price']}\n"
